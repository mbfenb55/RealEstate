...
export function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  if (!service) {
    return {
      ...
      description: "Parselim hizmet detaylari"
    };
  }
  ...
  return {
    ...
    alternates: { canonical: `${appUrl}/hizmetler/${service.slug}` }
  };
}
      ...
      <section className="container py-16">
        <div className="grid gap-10 lg:grid-cols-[1fr_360px]">
          <div className="space-y-6">
            <p className="text-sm font-medium uppercase tracking-[0.2em] text-primary">Hizmet Detayi</p>
              ...
              {service.highlights.map((highlight) => (
                <Card key={highlight} className="rounded-[2rem]">
                  <CardContent className="p-6">
                    <p className="text-sm text-muted-foreground">One cikan modul</p>
          ...
          <Card className="h-fit rounded-[2rem]">
            <CardContent className="space-y-4 p-6">
              <p className="text-sm text-muted-foreground">Baslangic fiyati</p>
              <p className="text-4xl font-semibold">{formatCurrency(service.priceFrom)}</p>
              <p className="text-sm leading-6 text-muted-foreground">
                Satisa hazir video tur, rapor ozeti ve teslim baglantisi dahildir.
              </p>
              <Link
                href="/dashboard/yeni-cekim"
